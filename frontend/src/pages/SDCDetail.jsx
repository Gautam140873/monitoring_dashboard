              ...
              <div className="space-y-6">
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge className="text-lg px-3 py-1">
                        {processStatus.overall_progress}%
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <Progress value={processStatus.overall_progress} className="h-3" />
                    <p className="text-sm text-muted-foreground mt-2">
                      Target: {processStatus.target_students} students
                    </p>
                  </CardContent>
                </Card>

                {/* Process Stages - Sequential Flow */}
                <Card className="border border-border">
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sequential stages: Mobilization → Training → OJT → Assessment → Placement
                    </p>
                  </CardHeader>
                  <CardContent>
                            ...
                            <div className="mb-3">
                              <div className="flex justify-between text-sm mb-1">
                                ...
                                <span className="font-mono font-medium">
                                  {stage.completed}/{stage.target} ({stage.progress_percent}%)
                                </span>
                              </div>
                              <Progress 
                                value={stage.progress_percent} 
                            ...
                            {!isLocked && user?.role === "ho" && (
                              <div className="flex gap-2 mt-3">
                                ...
                                <Button 
                                  variant="outline" 
                                  size="sm"
                                  onClick={() => handleUpdateStage(stage.stage_id, { completed: stage.target })}
                ...
                <Card className="border border-border">
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Material distribution and other deliverables
                    </p>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {processStatus.deliverables?.map((deliv) => (
                        ...
                        >
                          <div className="font-medium mb-2">{deliv.name}</div>
                          ...
                          {user?.role === "ho" ? (
                            ...
                            <Badge variant={
                              deliv.status === "completed" ? "default" :
                              deliv.status === "not_required" ? "secondary" : "outline"
                            } className={
                              deliv.status === "completed" ? "bg-emerald-100 text-emerald-700" : ""
                            }>
                              {deliv.status === "completed" ? "✅ Done" :
                               deliv.status === "not_required" ? "➖ N/A" : "⏳ Pending"}
                            </Badge>
                          ...
                          {deliv.completed_date && (
                            <div className="text-xs text-muted-foreground mt-1">
                              {new Date(deliv.completed_date).toLocaleDateString()}
                            </div>
                          )}
                        </div>
                      ))}
